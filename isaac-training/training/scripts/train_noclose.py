...
def main(cfg):
    ...
    checkpoint = "/home/ubuntu/projects/NavRL/isaac-training/ckpts/checkpoint_final_5e6.pt"
    policy.load_state_dict(torch.load(checkpoint, map_location=cfg.device))
    print(f"[RESUME] loaded checkpoint: {checkpoint}")
    ...
    print(f"[DEBUG] cfg.max_frame_num={cfg.max_frame_num}")
    print(f"[DEBUG] cfg.algo.training_frame_num={cfg.algo.training_frame_num}")
    print(f"[DEBUG] cfg.env.num_envs={cfg.env.num_envs}")
    print(f"[DEBUG] frames_per_batch={cfg.env.num_envs * cfg.algo.training_frame_num}")

    # Training Loop
    for i, data in enumerate(collector):
        if i < 5 or i % 100 == 0:
            print(f"[DEBUG] loop_i={i}, collector_frames={collector._frames}")
        # print("data: ", data)
        # print("============================")
        # Log Info
        ...
        if i > 0 and i % cfg.eval_interval == 0:
            ...
            print(f"[EVAL] step={i}, env_frames={collector._frames}, eval_info={eval_info}")
